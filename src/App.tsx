...
import { 
  ...
  ArrowUp, Sun, Moon, MessageSquare, ExternalLink, Globe
} from "lucide-react";
...
import { getFirestore, collection, addDoc, onSnapshot, query as fsQuery, orderBy, limit as fsLimit, getDocs, deleteDoc, doc } from "firebase/firestore";

// Import custom structured types, components, and presets
import { Message, ChatSession, FirebaseConfigType } from "./types";
import { OFFLINE_PRESETS } from "./data/presets";
import { MessageBubble } from "./components/MessageBubble";
import { PresetButton } from "./components/PresetButton";
import { SidebarSessionItem } from "./components/SidebarSessionItem";

// --- DEEP SEARCH PROCESS PROGRESS ANIMATION ---
function DeepSearchLoader() {
  const [currentStep, setCurrentStep] = useState(0);
  const steps = [
    "🔍 Analyzing quantum parameters & query intent...",
    "🌐 Initializing Deep Web crawl & indexing astronomical database...",
    "🔬 Cross-referencing high-authority research papers in parallel...",
    "🧠 Performing multi-dimensional matrix synthesis...",
    "🌌 Finalizing comprehensive Singularity report output..."
  ];
  ...
  return (
    <div className="space-y-2 font-mono pt-1">
      {steps.slice(0, currentStep + 1).map((step, idx) => (
        <div key={idx} className="flex items-center gap-2 text-[10.5px] transition-all duration-300">
          {idx === currentStep ? (
            <span className="w-1.5 h-1.5 rounded-full bg-purple-500 animate-ping" />
          ) : (
            <Check className="w-3 h-3 text-emerald-400 shrink-0" />
          )}
          <span className={idx === currentStep ? "text-purple-300 font-semibold" : "text-gray-400"}>
            {step}
          </span>
        </div>
      ))}
...
export default function App() {
  ...
  const [customApiUrl, setCustomApiUrl] = useState<string>(() => {
    return localStorage.getItem("IBT_X_GEM_API_BASE_URL") || "";
  });

  const [activeProvider, setActiveProvider] = useState<"gemini" | "alternative">(() => {
    return (localStorage.getItem("IBT_X_GEM_ACTIVE_PROVIDER") as "gemini" | "alternative") || "gemini";
  });
  ...
  const [deepSearch, setDeepSearch] = useState<boolean>(() => {
    return localStorage.getItem("IBT_X_GEM_DEEP_SEARCH") === "true";
  });
  ...
  const [tokens, setTokens] = useState<number>(() => {
    ...
    const hasAnotherKey = !!localStorage.getItem("IBT_X_GEM_ANOTHER_KEY") || localStorage.getItem("IBT_X_GEM_ACTIVE_PROVIDER") === "alternative";
    const hasBoosted = localStorage.getItem("IBT_X_GEM_HAS_ANOTHER_AI_BOOST") === "true";

    if (saved) {
      let currentVal = parseInt(saved, 10);
      if (hasAnotherKey && !hasBoosted) {
        currentVal += 5550;
        localStorage.setItem("IBT_X_GEM_HAS_ANOTHER_AI_BOOST", "true");
        localStorage.setItem("IBT_X_GEM_TOKENS", String(currentVal));
      }
      return currentVal;
    }
  ...
  useEffect(() => {
    localStorage.setItem("IBT_X_GEM_DEEP_SEARCH", String(deepSearch));
  }, [deepSearch]);
  ...
  useEffect(() => {
    localStorage.setItem("IBT_X_GEM_ACTIVE_PROVIDER", activeProvider);
  }, [activeProvider]);
  ...
  useEffect(() => {
    const hasBoosted = localStorage.getItem("IBT_X_GEM_HAS_ANOTHER_AI_BOOST") === "true";
    if ((anotherApiKey.trim() !== "" || activeProvider === "alternative") && !hasBoosted) {
      setTokens(prev => {
        const newVal = prev + 5550;
        localStorage.setItem("IBT_X_GEM_TOKENS", String(newVal));
        return newVal;
      });
      localStorage.setItem("IBT_X_GEM_HAS_ANOTHER_AI_BOOST", "true");
      triggerNotification("🌌 Quantum Boost: +5550 Token gratis diaktifkan karena menggunakan API AI lain!", "success");
    }
  }, [anotherApiKey, activeProvider, triggerNotification]);
  ...
  const saveGeneralConfigs = useCallback((
    ...
    if (updatedCustomApiUrl !== undefined) {
      localStorage.setItem("IBT_X_GEM_API_BASE_URL", updatedCustomApiUrl);
      setCustomApiUrl(updatedCustomApiUrl);
    }
  ...
  const handleClearSessionLocal = useCallback(async () => {
    ...
    try {
      ...
      if (activeDbEngine === "RTDB") {
        const db = getDatabase(appInstance);
        const chatRef = rtdbRef(db, `ibt_chats/${activeSessionId}/messages`);
        await rtdbSet(chatRef, null);
        setMessages([]);
        triggerNotification("Chat history cleared from Realtime Database", "success");
        ...
        querySnapshot.forEach((document) => {
          const docRef = doc(db, "ibt_chats", activeSessionId, "messages", document.id);
          deletePromises.push(deleteDoc(docRef));
        });
        
        await Promise.all(deletePromises);
        setMessages([]);
        triggerNotification("Chat history cleared from Firestore Database", "success");
      }
    } catch (e: any) {
      console.error("Failed to clear database logs:", e);
      triggerNotification(`Failed to clear database: ${e.message || e}`, "error");
    }
  }, [activeDbEngine, activeSessionId, playTactileBeep, triggerNotification]);
  ...
  const handleSendMessage = useCallback(async (textToSend?: string) => {
    ...
    if (matchedPreset) {
      setTimeout(async () => {
        ...
        // Substract tokens (always active to keep token system fully functional)
    ...
    // Dynamic Google Gemini & Alternative AI Proxy endpoint pipeline
    try {
      const contextMessages = messages.concat(userMsgPayload).slice(-12); // Send last 12 messages for memory

      const apiBase = (localStorage.getItem("IBT_X_GEM_API_BASE_URL") || "").trim().replace(/\/$/, "");
      const endpoint = apiBase ? `${apiBase}/api/gemini/chat` : "/api/gemini/chat";
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          activeProvider: activeProvider, // "gemini" or "alternative"
          anotherProvider: anotherProvider, // "openai", "anthropic", "deepseek", "cohere", "groq"
          anotherApiKey: anotherApiKey,
      ...
      // Subtract tokens (always active to keep token system fully functional)
      ...
      const sources = data.groundingChunks 
        ? data.groundingChunks.map((chunk: any) => ({
      ...
      try {
        if (activeDbEngine === "RTDB") {
          ...
          await rtdbSet(newMsgRef, {
            ...
            text: aiText,
            timestamp: Date.now(),
            searchSources: sources.length > 0 ? sources : null
          });
        } else if (activeDbEngine === "Firestore") {
          const appInstance = getApp();
          const db = getFirestore(appInstance);
    ...
    <div className={`relative min-h-screen flex flex-col font-sans selection:bg-purple-900/60 selection:text-white transition-all duration-300 ${
      ...
      <div className="relative z-10 flex-1 flex flex-col lg:flex-row h-screen overflow-hidden">
        ...
        <main className="flex-1 flex flex-col bg-transparent relative h-full overflow-hidden">
          ...
          {/* Header */}
          <header className="sticky top-0 z-20 px-6 py-4 border-b border-white/5 bg-[#050507]/90 backdrop-blur-md flex items-center justify-between shadow-lg shadow-black/20">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-400 font-mono tracking-wider">
                  ENGINE: <span className="text-blue-400 font-bold uppercase">{activeProvider}</span> | MODEL: <span className="text-purple-400 font-semibold">{activeProvider === "alternative" ? (anotherModel || `${anotherProvider} (default)`) : activeModel}</span>
                </p>
              </div>
            </div>
          ...
          {/* Non-Dismissible Ads Banner */}
          <div 
            onClick={() => window.open("https://blackholeai-api-production.up.railway.app", "_blank")}
            className="cursor-pointer bg-gradient-to-r from-purple-950/40 via-pink-950/20 to-blue-950/40 border-b border-purple-500/20 py-2.5 px-6 text-center transition-all hover:from-purple-900/50 hover:via-pink-900/30 hover:to-blue-900/50 relative z-10 flex items-center justify-center gap-2 group select-none shadow-sm shadow-purple-500/5"
          >
            <div className="absolute inset-0 bg-gradient-to-r from-purple-500/5 via-pink-500/5 to-blue-500/5 animate-pulse" />
            <Sparkles className="w-4 h-4 text-yellow-400 shrink-0 group-hover:scale-110 transition-transform duration-300" />
            <span className="text-xs sm:text-sm font-bold tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-purple-300 via-pink-200 to-yellow-200 uppercase font-mono group-hover:text-glow-purple transition-all duration-300">
              Get your own Ai Api
            </span>
            <ExternalLink className="w-3.5 h-3.5 text-pink-400 shrink-0 opacity-80 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all duration-300" />
          </div>
          ...
          >
            ...
            <div className="max-w-3xl mx-auto space-y-6">
              ...
              {isAiLoading && (
                <div className="flex justify-start animate-none">
                  <div className="glass-panel-neon rounded-2xl rounded-bl-none p-5 max-w-[85%] space-y-3">
                    <div className="flex items-center gap-1.5 text-[10px] font-mono text-purple-400">
                      <Sparkles className="w-3 h-3 animate-spin" />
                      <span>{deepSearch ? "DEEP RESEARCH PROTOCOL ACTIVE" : "Singularity resolving gravity warp..."}</span>
                    </div>
                    {deepSearch ? (
                      <DeepSearchLoader />
                    ) : (
              ...
              >
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    type="button"
                    onClick={() => {
                      playTactileBeep(deepSearch ? 200 : 350, 0.08);
                      setDeepSearch(!deepSearch);
                    }}
                    className={`px-3 py-1.5 rounded-xl text-xs font-mono font-bold tracking-wider uppercase border transition-all flex items-center gap-1.5 cursor-pointer ${
                      deepSearch
                        ? "bg-purple-950/40 text-purple-300 border-purple-500/50 shadow-md shadow-purple-500/10"
                        : "bg-white/5 text-gray-400 border-transparent hover:bg-white/10"
                    }`}
                    title="Toggle Deep Search & Web Grounding"
                  >
                    <Globe className={`w-3.5 h-3.5 ${deepSearch ? "text-indigo-400 animate-spin" : ""}`} style={deepSearch ? { animationDuration: '6s' } : undefined} />
                    <span className="hidden sm:inline">Deep Search</span>
                    <span className={`w-1.5 h-1.5 rounded-full ${deepSearch ? "bg-purple-400 animate-ping" : "bg-gray-600"}`} />
                  </button>
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 py-6 space-y-6 text-sm">
                ...
                {/* Active AI Provider Toggle Selection */}
                <div className="space-y-3 p-4 rounded-xl border border-blue-900/35 bg-blue-950/10">
                  <div className="flex items-center gap-2 text-blue-300 font-semibold font-display">
                    <Sparkles className="w-4.5 h-4.5 text-blue-400" />
                    <span>Primary AI Chat Engine / Provider</span>
                  </div>
                  <p className="text-[11px] text-gray-400 leading-normal">
                    Pilih apakah ingin menggunakan Google Gemini Core sebagai AI utama Anda, atau beralih ke Provider AI alternatif yang dikonfigurasi di bawah.
                  </p>
                  <select
                    value={activeProvider}
                    onChange={(e) => {
                      const val = e.target.value as "gemini" | "alternative";
                      setActiveProvider(val);
                      playTactileBeep(440, 0.05);
                      triggerNotification(`Primary Chat Engine: ${val === "gemini" ? "Google Gemini Core" : "Alternative AI Provider"}`, "success");
                    }}
                    className="w-full px-2.5 py-2 bg-black border border-white/10 rounded-lg text-xs text-white focus:outline-none focus:border-blue-500 cursor-pointer font-bold font-mono uppercase tracking-wide"
                  >
                    <option value="gemini">🪐 Google Gemini Core (Default)</option>
                    <option value="alternative">🚀 Alternative AI Provider (OpenAI, Claude, DeepSeek, etc.)</option>
                  </select>
                </div>
                ...
                {/* Custom API Base URL for mobile packaging/Railway fallback */}
                <div className="space-y-2 p-3 bg-white/[0.02] border border-white/5 rounded-xl">
                  <div className="flex items-center gap-2 text-white font-semibold font-display">
                    <Globe className="w-4.5 h-4.5 text-emerald-400" />
                    <span>Custom API Backend URL</span>
                  </div>
                  <p className="text-[11px] text-gray-400 leading-normal">
                    Digunakan jika di-package ke Android agar bisa terhubung dengan server Railway atau Vercel Anda (misalnya <code className="text-emerald-300 font-mono">https://blackhole-gemini.up.railway.app</code>). Biarkan kosong jika menggunakan web local proxy default.
                  </p>
                  <input
                    type="text"
                    value={customApiUrl}
                    onChange={(e) => setCustomApiUrl(e.target.value)}
                    placeholder="https://your-deployed-app.railway.app"
                    className="w-full px-2.5 py-1.5 bg-black border border-white/10 rounded-lg text-white font-mono text-xs focus:outline-none focus:border-emerald-500"
                  />
                </div>
